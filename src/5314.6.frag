...
float scene(vec3 p) { 
  float ba = udBox(opRep(p,vec3(8.5)),vec3(2.));
  float bb = udBox(opRep(p,vec3(6.7)),vec3(1.4));
  float sa = sdSphere(opRep(p,vec3(4.)),1.65);
	
  float d = min(max(-sa,ba),bb);
  return clamp(fbm(p),0.,1.)*0.25+d;
}
...
void main( void ) {
  vec3 e = vec3(time*1.5,time*2.,time); 
  vec3 p = vec3((gl_FragCoord.xy / resolution.xy) * 2. -1., 0.5);
