/*
 * Original shader from: https://www.shadertoy.com/view/4sBGD1
 */
...
vec4 hash4( const in vec3 p ) {
    float h = dot(p,vec3(1.0,113.0,758.5453123));
    return fract(sin(vec4(h, h+1.0, h+2.0, h+3.0))*43758.5453123);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	{
		vec4 rr = hash4(vec3(fragCoord, float(a)));
        vec2 p = -1.0 + 2.0*(fragCoord.xy+rr.xz) / iResolution.xy;
        p.x *= iResolution.x/ iResolution.y;
        float time = 16.0*(iTime + (0.5/24.0)*rr.w);
