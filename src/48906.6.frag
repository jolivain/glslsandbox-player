...
{
    p=fract(p/256.0);
    return texture2D(mySampler, p, 0.0).r;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    else
    {
        ...
        dl*=smoothstep(20.0,900.0,distance(rp,vec3(1905.0,3728.0,1046.0)));
        {
            ...
            dl+=(0.9+0.1*cos(iTime *15.0))*sh*2.0*max(0.0,0.2+d3*17.0)*vec3(1.0,0.65,0.3)*(1.0-smoothstep(0.0,280.0,distance(lp,vec3(225.0,0.0,0.0))));
        }
    ...
    // Render the torch flames and their reflections.
    {
        ...
        {
            vec3 ff=fire(frp.xy,iTime )*step(0.0,ft);
    ...
    fragColor.rgb+=vec3(1.0,0.6,0.3)*(1.0-smoothstep(0.0,0.5,abs(mod(iTime *0.5,29.0)-1.5)))*0.12;
