/*
 * Original shader from: https://www.shadertoy.com/view/XddfDr
 */
...
float wave(float n, float x, float l, float phase){
    float sw = mod(iTime, duration*3.0) / duration;
    if (sw >= 0. && sw < 1.)
        return squarewave(n,x,l,phase);
    else if (sw >= 1. && sw < 2.)
        return sawtoothwave(n,x,l,phase);
    else
        return trianglewave(n,x,l,phase);
}
...
float dwave(float n, float x, float l, float phase){
    float sw = mod(iTime, duration*3.0) / duration;
    if (sw >= 0. && sw < 1.)
        return dsquarewave(n,x,l,phase);
    else if (sw >= 1. && sw < 2.)
        return dsawtoothwave(n,x,l,phase);
    else
        return dtrianglewave(n,x,l,phase);
}
// =======================================
